export default async function handler(req, res) {
  ...
  // Haalt ruwe slotkoersen op — gebruikt voor hist (line chart rebasing)
  async function fetchClose(symbol, period1) {
  ...
  // Haalt voorberekende % op vanaf begin periode — gebruikt voor ytd (bar chart)
  async function fetchPct(symbol, period1) {
    const valid = await fetchClose(symbol, period1);
      ...
      tickers.map(async ({ symbol, name, group }) => {
        try {
          const [ytd, hist] = await Promise.all([
            fetchPct(symbol, period1YTD),   // % vanaf 1 jan — voor bar chart
            fetchClose(symbol, period1Hist), // ruwe prijzen vanaf 2020 — voor line chart
          ]);
